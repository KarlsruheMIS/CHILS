...
namespace mwis
{
  ...
  template <typename T>
  bool feasibility_check(const T sum) { return std::abs(sum - 1.0) < 1e-8; }
  template <>
  bool feasibility_check(const int sum) { return sum == 1; }

  class solver
  {
  public:
    solver()
        : finalized_graph_(false), finalized_costs_(false), constant_(0.0), scaling_(1.0), gen_(42)
#ifdef ENABLE_QPBO
          ,
          qpbo_(0, 0)
#endif
             ...
             const double min_duality_gap)
    {
      assert(finalized());
      //** signal_handler h;
      //** dbg::timer t_total, t_dual, t_primal;
      ...
      for (int i = 0; i < max_batches; ++i)
      {
        ...
        if (gap_relaxed < min_duality_gap)
        {
          ...
          //** t_primal.start();
          // update_integer_assignment(greedy_generations); //standard code
          // OUTPUT greedy solutions in JSON format: begin ===============================
          // std::cout << "{";
          // for (int i = 0; i < greedy_generations; ++i)
          // {
          //   greedy();
          //  // uncomment the line below and commen out the next one if you need binary representation
          //  // print_n_elements_as_json_binary(assignment_latest_.begin(), no_orig(), "solution-" + std::to_string(i + 1));
          //   print_n_elements_as_json(assignment_latest_.begin(), no_orig(), "solution-" + std::to_string(i + 1),no_cliques());
          //   if (i < greedy_generations-1) std::cout << ", ";
          // }
          // std::cout << "}" << std::endl;
          // //** t_primal.stop();
          break;
        ...
        //           //** << "t_dual=" << t_dual.seconds<true>() << "s "
        //           //** << "t_primal=" << t_primal.seconds<true>() << "s "
        //           //** << "total=" << t_total.milliseconds<true>() / iterations_ << "ms/it"
        //           << std::endl;
      ...
      //           //** << "total time=" << t_total.seconds<true>() << "s. "
      //           << std::endl;
    }
  ...
  protected:
    ...
    cost dual_smoothed_scaled() const
    {
      ...
      auto f = [this](const auto a, const auto c)
      { return a + std::exp(c / temperature_); };
